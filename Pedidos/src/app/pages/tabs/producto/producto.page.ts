...
export class ProductoPage implements OnInit {
  ...
  carrito(){
    this.router.navigate(['/carrito'] ,{ queryParams: {  bolsa : JSON.stringify(this.bolsa)}} );
  }

  adicionarCarrito(producto){
     if (this.bolsa.length>0) {
        let i = this.bolsa.filter( bus=> bus.producto.idProducto == producto.idProducto);
        if (i.length > 0) {
          this.presentToast('Producto ya existe en la Bolsa.');
        } else {
            this.bolsa.push({ producto : producto , cantidad : 1  })
